...
A pirateplot() is the RDI (**Raw** data, **Descriptive** statistics, and **Inferential** statistics) plotting choice of R pirates who are displaying the relationship between 1 or two categorical independent variables, and one continuous dependent variable.
...
yarrr::pirateplot(formula = time ~ cleaner + type,
           data = poopdeck,
...
`theme.o = 0` will turn off all of the opacities. You can then manually turn on each one with the x.o arguments (e.g.; `bean.o`, `point.o`, `line.o`)
